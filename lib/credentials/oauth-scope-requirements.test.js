...
import {
  ACTIVEPIECES_SHARED_GMAIL_SCOPES,
  activepiecesSharedOAuthCompatibility,
...
test('accepts Gmail operations covered by Activepieces shared OAuth', () => {
  const workflow = {
    nodes: [
      { name: 'Watch', type: 'n8n-nodes-base.gmailTrigger', parameters: {} },
      { name: 'Draft', type: 'n8n-nodes-base.gmail', parameters: { resource: 'draft' } },
  ...
  const result = activepiecesSharedOAuthCompatibility(workflow, {
    connectorId: 'gmail',
    nodeNames: ['Watch', 'Draft', 'Send'],
  });

  assert.equal(result.supported, true);
  assert.deepEqual(ACTIVEPIECES_SHARED_GMAIL_SCOPES, [
    GMAIL_OAUTH_SCOPES.readonly,
    GMAIL_OAUTH_SCOPES.compose,
    GMAIL_OAUTH_SCOPES.send,
  ]);
...
test('rejects Gmail modification before connection or execution', () => {
  const workflow = {
    nodes: [{
      name: 'Label message',
      type: 'n8n-nodes-base.gmail',
  ...
  const requirement = {
    connectorId: 'gmail',
    nodeNames: ['Label message'],
  };

  assert.throws(
    () => assertActivepiecesSharedOAuthCompatibility(workflow, requirement),
    (error) => (
      error.code === 'ACTIVEPIECES_OAUTH_SCOPE_UNSUPPORTED' &&
      error.affectedNodes.includes('Label message') &&
      error.unsupportedScopes.includes(GMAIL_OAUTH_SCOPES.modify)
    ),
  );
});
