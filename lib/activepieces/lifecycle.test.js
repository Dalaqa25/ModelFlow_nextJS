...
test('configures a completed Activepieces event workflow after setup', () => {
  const automation = {
    activepieces_source_flow_id: 'flow-1',
  };
  const lifecycle = {
  ...
  assert.equal(shouldConfigureActivepiecesAutomation({ automation, lifecycle }), true);
  assert.equal(shouldConfigureActivepiecesAutomation({
    automation: { activepieces_source_flow_id: null },
    lifecycle,
  }), false);
  assert.equal(shouldConfigureActivepiecesAutomation({
    automation,
    lifecycle: { ...lifecycle, setupComplete: false },
  }), false);
});
