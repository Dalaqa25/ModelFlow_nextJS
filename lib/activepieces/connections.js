...
import { callNativeAutomationRuntime } from '../automation-runtime/client.js';
import {
  assertActivepiecesSharedOAuthCompatibility,
...
export function humanizePiece(pieceName) {
  const slug = getPieceSlug(pieceName);
...
async function listImportedConnectionStatus({ supabase, user, automation }) {
  ...
  const requirements = detected.map((requirement) => {
    ...
    const requiresSharedOAuth = requirement.connectorId === 'gmail';
    const usesActivepiecesSharedOAuth =
      String(existingConnection?.type || '').toUpperCase() === 'CLOUD_OAUTH2';
    return {
      ...requirement,
      externalId,
      pieceVersion: null,
      ...
      actionName: null,
      connected: isUsableAppConnection(existingConnection) &&
        (!requiresSharedOAuth || usesActivepiecesSharedOAuth),
      status: existingConnection?.status || 'missing',
      existingConnectionId: existingConnection?.id || null,
      needsReconnect: Boolean(
        isUsableAppConnection(existingConnection) &&
        requiresSharedOAuth &&
        !usesActivepiecesSharedOAuth
      ),
    };
  });
...
export async function prepareActivepiecesConnectionStart({ supabase, user, automation, externalId = null }) {
  ...
  // Imported workflows must use Activepieces' shared OAuth application. A
  // ModelGrow-owned OAuth client would bypass the marketplace scope policy.
  const modelGrowOAuthApp = automation.activepieces_source_flow_id
    ? getModelGrowOAuthApp(target.pieceName)
    : null;
  ...
  const pieceScopes = mergeOAuthScopes(Array.isArray(authOption?.scope) ? authOption.scope : []);
  if (!automation.activepieces_source_flow_id && !modelGrowOAuthApp) {
    assertActivepiecesSharedOAuthCompatibility(workflow, target, pieceScopes);
  }
  const scopes = modelGrowOAuthApp
    ? getConnectionOAuthScopes({ automation, target, authOption })
    : pieceScopes;
  const props = {};
...
// Who a connection actually belongs to, and whether that is likely a mistake.
//
// A Google consent screen offers whichever account the browser is already
// signed into, and every connection here is labelled only by its piece —
// "Gmail" — so someone can connect the wrong mailbox with nothing anywhere to
// contradict them. The automation then polls an inbox they never read and
// reports no error, because from its side nothing is wrong. Only the credential
// bridge holds a usable token, and only the runtime can reach the bridge, so
// the answer has to be fetched rather than read locally.
//
// A notice, never an obstacle. Connecting an account that differs from the one
// someone signed up with is ordinary and often deliberate — a work mailbox
// automated from a personal login. The flag exists so the interface can show
// the address plainly, not so it can refuse.
async function describeConnectedAccount({ user, target, projectId, externalId }) {
  let account = null;
  try {
    const result = await callNativeAutomationRuntime('/api/connections/identity', {
      connector_id: target?.connectorId
        || target?.pieceName?.split('/').pop()?.replace(/^piece-/, '')
        || null,
    ...
    // The runtime being unreachable costs a label, not the connection.
    return null;
  }
  ...
  return {
    account,
    matchesSignedInUser: signedInAs ? account.toLowerCase() === signedInAs : null,
  };
}
...
export async function completeActivepiecesOAuthConnection({ supabase, user, automation, externalId, pieceName, pieceVersion, clientId, code, codeVerifier, authorizationMethod = null, props = {} }) {
  ...
  const modelGrowOAuthApp = automation.activepieces_source_flow_id
    ? getModelGrowOAuthApp(pieceName)
    : null;
  ...
  return {
    connection,
    accountNotice: await describeConnectedAccount({
      user,
      target,
      projectId: status.projectId,
      externalId,
