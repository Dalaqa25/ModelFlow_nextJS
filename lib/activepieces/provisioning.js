...
import { recordSuccessfulAutomationUsage } from '@/lib/billing/automation-usage.js';
...
// ModelGrow's own owner/service login to the Builder was rejected. This is an
// operator problem (rotated or stale ACTIVEPIECES_OWNER_PASSWORD), never a
// problem caused by the end user, so callers should not blame user state for it.
export function isActivepiecesPlatformAuthError(error) {
  return (
    error?.status === 401 &&
...
export async function ensureActivepiecesUserForModelGrowUser({ supabase, user }) {
  ...
  try {
    ...
    // A broken ModelGrow-to-Builder service login is a platform outage, not a
    // problem with this user's link. Recording it as `failed` would brand every
    // user who happened to try setup during the outage, and those rows would
    // outlive the outage itself. Let the error surface without touching state.
    if (isActivepiecesPlatformAuthError(error)) {
      throw error;
    }
...
export async function recordSuccessfulTokenSpend({
  ...
  return recordSuccessfulAutomationUsage({
    supabase,
    user,
    automation,
    tokenCost,
