...
async function getAutomationRecord({ supabase, automationId }) {
  const { data, error } = await supabase
    .from('automations')
    .select('id, name, activepieces_source_flow_id, activepieces_source_project_id, activepieces_trigger_type')
    .eq('id', automationId)
    .maybeSingle();
...
export async function getActivepiecesRuntimeStatus({ supabase, user, automationId, limit = 5 }) {
  ...
  if (!automation.activepieces_source_flow_id) {
    return {
      ...
      recentRuns: [],
      duplicateBurst: null,
      message: 'This automation is not powered by ModelGrow Builder.',
    };
  }
