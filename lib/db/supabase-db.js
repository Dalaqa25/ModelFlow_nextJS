  ...
  async getAllRequests() {
    const { data, error } = await supabase
      ...
      .select(`
        *,
        ...
        author:users!requests_author_email_fkey(name, profile_image_url)
      `)
      .order('created_at', { ascending: false });
    ...
    if (error) {
      ...
      const { data: fallback, error: fallbackError } = await supabase
        .from('requests')
        .select(`*, request_comments(count)`)
        .order('created_at', { ascending: false });
      if (fallbackError) throw fallbackError;
      return fallback.map(request => ({
        ...request,
        commentsCount: request.request_comments?.[0]?.count || 0,
        author_name: request.author_email?.split('@')[0] || 'User'
      }));
    }
    ...
    return data.map(request => ({
      ...request,
      commentsCount: request.request_comments?.[0]?.count || 0,
      author_name: request.author?.name || request.author_email?.split('@')[0] || 'User'
    }));
  },
