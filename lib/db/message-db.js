...
export const messageDB = {
  ...
  async findThreadBetweenUsers(userIdA, userIdB) {
    // Find all threads that A belongs to
    const { data: membershipsA, error: errA } = await supabase
      .from('dm_participants')
      .select('thread_id')
      .eq('user_id', userIdA);
    
    if (errA) throw errA;
    if (!membershipsA?.length) return null;

    const threadIdsA = membershipsA.map(m => m.thread_id);

    // Filter threads that B also belongs to
    const { data: membershipsB, error: errB } = await supabase
      .from('dm_participants')
      .select('thread_id')
      .eq('user_id', userIdB)
      .in('thread_id', threadIdsA);
      
    if (errB) throw errB;
    if (!membershipsB?.length) return null;

    // Get the first active or pending thread between them
    const sharedThreadId = membershipsB[0].thread_id;
    
    const { data: thread, error } = await supabase
      .from('dm_threads')
      .select('id, status, request_id, created_by_user_id, created_at, updated_at')
      .eq('id', sharedThreadId)
      .neq('status', 'declined')
      .maybeSingle();
