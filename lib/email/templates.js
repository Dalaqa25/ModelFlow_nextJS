...
/**
 * Email sent to admins when a developer submits an automation for review.
 */
export function automationReviewRequestedEmail({
  automationName,
  automationId,
  authorEmail,
  description,
  ...
  const reviewUrl = `${APP_URL}/admin/withdrawals?tab=automations`;
  const safeName = escapeHtml(automationName || 'Untitled automation');
  const safeAuthor = escapeHtml(authorEmail || 'Unknown author');
  const safeSource = escapeHtml(source || 'Upload');
  const safeDescription = escapeHtml(truncate(description || 'No description provided.')).replace(/\n/g, '<br />');
  const safeAutomationId = escapeHtml(automationId || '');

  const html = layout(`
    <p style="margin:0 0 12px;font-size:15px;color:#94a3b8;line-height:1.6;">
      A new automation is waiting for admin review.
    </p>
    <h2 style="margin:0 0 18px;font-size:22px;line-height:1.35;color:#e2e8f0;font-weight:700;">
      ${safeName}
    </h2>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 18px;border-collapse:collapse;">
      <tr>
        <td style="padding:8px 0;font-size:13px;color:#64748b;width:110px;">Author</td>
        <td style="padding:8px 0;font-size:13px;color:#e2e8f0;">${safeAuthor}</td>
      </tr>
      <tr>
        <td style="padding:8px 0;font-size:13px;color:#64748b;width:110px;">Source</td>
        <td style="padding:8px 0;font-size:13px;color:#e2e8f0;">${safeSource}</td>
      </tr>
      ${safeAutomationId ? `
      <tr>
        <td style="padding:8px 0;font-size:13px;color:#64748b;width:110px;">Automation ID</td>
        <td style="padding:8px 0;font-size:13px;color:#e2e8f0;font-family:monospace;">${safeAutomationId}</td>
      </tr>` : ''}
    </table>
    <p style="margin:0 0 10px;font-size:13px;font-weight:700;color:#cbd5e1;text-transform:uppercase;letter-spacing:0.08em;">
      Description
    </p>
    <p style="margin:0;font-size:15px;color:#94a3b8;line-height:1.75;">
      ${safeDescription}
    </p>
    ${button(reviewUrl, 'Review automation')}
  `);

  return {
    subject: `Review needed: ${automationName || 'Untitled automation'}`,
    html,
  };
}
