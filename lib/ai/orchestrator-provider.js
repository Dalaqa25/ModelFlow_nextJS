...
// `transform` maps the raw structured output to whatever the caller wants.
// Orchestrator decisions normalize against the allowed action list; other
// callers — filling in tool arguments, for one — want the schema's own object
// back untouched, so the shape stays their business rather than this
// function's. `systemPrompt` and `model` are likewise overridable, since a
// decision call and an extraction call want neither the same instructions nor
// necessarily the same tier of model.
async function executeLocalClaude(prompt, timeoutMs, outputSchema, allowedActions, options = {}) {
  const transform = options.transform
    || (structured => normalizeOrchestratorDecision(structured, allowedActions));
  ...
  const args = [
    ...
    '--model', options.model?.trim() || process.env.CLAUDE_MODEL?.trim() || 'claude-sonnet-5',
    ...
    '--setting-sources', '',
    '--strict-mcp-config',
    '--system-prompt', options.systemPrompt || CODEX_BASE_INSTRUCTIONS,
  ];
...
// Fill in one tool's arguments with Claude Code, against the tool's own JSON
// schema. This is the same job an OpenAI tool call does, and `--json-schema`
// does it directly, so no tool-calling protocol is involved — the schema goes
// in and a conforming object comes back.
//
// Chosen over a hosted chat model because the account already pays for Claude
// Code, and because the retirement of GitHub Models took this exact call down
// with it: a provider ModelGrow does not control failing on a schedule
// ModelGrow does not set. CLAUDE_TOOL_MODEL defaults to Haiku since filling in
// arguments from an explicit schema is a small extraction task, and this call
// sits in the setup path where latency shows.
export async function createClaudeToolArguments({ prompt, schema, systemPrompt }) {
  const startedAt = Date.now();
  const timeoutMs = positiveInteger(process.env.CLAUDE_TOOL_TIMEOUT_MS, DEFAULT_CODEX_TIMEOUT_MS);
  const args = await executeLocalClaude(prompt, timeoutMs, schema, null, {
    systemPrompt,
    model: process.env.CLAUDE_TOOL_MODEL || 'claude-haiku-4-5-20251001',
    transform: structured => structured,
  });
  console.log(`[AI] Claude tool arguments in ${Date.now() - startedAt}ms`);
  return args;
}
