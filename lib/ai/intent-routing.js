...
export function extractAvailableAutomationDiscoveryRequest(content, messages = []) {
  ...
  // browseAll is a preference, not an instruction. The handler searches first
  // regardless and only falls back to a sample when nothing in the catalogue
  // matches, so a misread here costs nothing. Getting this wrong used to
  // discard the query outright: "some automation which replies to my emails"
  // matched on "some automation" and returned the first six rows.
