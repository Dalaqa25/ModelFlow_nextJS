...
export function extractAvailableAutomationDiscoveryRequest(content, messages = []) {
  ...
  // "some automations" reads as browsing on its own, but not when the sentence
  // goes on to say what the automation should do. "I need some automation which
  // replies to my emails" was matching the broad pattern on "some automation"
  // and returning the first six rows of the catalogue with the rest of the
  // sentence discarded — while "I need an automation which replies to my
  // emails" searched properly. One article's difference.
  //
  // A described outcome is the stronger signal, so it is checked first.
  // Naming a subject is enough — "some automation for my invoices" is as
  // targeted as "some automation which replies to my emails", it just skips the
  // verb. Browsing is what is left when nothing in the sentence narrows it.
  const namesASubject = /\b(?:email|emails|inbox|mail|message|messages|reply|replies|invoice|invoices|receipt|receipts|calendar|meeting|meetings|sheet|sheets|spreadsheet|doc|docs|document|documents|post|posts|content|caption|captions|lead|leads|customer|customers|client|clients|contact|contacts|task|tasks|ticket|tickets|report|reports|gmail|slack|notion|linkedin|telegram|airtable|sheet)\b/.test(normalized);

  // Only the subject counts here. describesAutomatedOutcome matches the word
  // "automate" on its own, which every broad phrasing contains — "what can
  // ModelGrow automate" would read as targeted and never browse.
  const describesWhatItShouldDo = namesASubject;

  if (broadCatalogRequest && !describesWhatItShouldDo) {
    return { browseAll: true, reason: 'explicit_catalog_request' };
  }
  if (broadCatalogRequest && describesWhatItShouldDo) {
