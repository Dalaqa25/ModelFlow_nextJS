...
export async function handleCollectTextInput(args, user, controller, encoder, setupContext = null) {
  ...
  const activeMissingFields = (setupContext?.missingFields || []).map((field) => (
    typeof field === 'string' ? field : field?.name || ''
  ).toUpperCase());
  const isExpectedSetupField = activeMissingFields.includes((args.field_name || '').toUpperCase());

  if (!isExpectedSetupField && (fieldLower.includes('folder') || fieldLower.includes('spreadsheet') || fieldLower.includes('sheet') || fieldLower.includes('document'))) {
  ...
  // CRITICAL: Clean the value to remove any leaked system prompts
  // These [ACTIVE SETUP: ...] blocks are internal context that should never be stored
  const cleanValue = (args.value || '').replace(/\s*\[ACTIVE SETUP:[\s\S]*?\]\s*/g, '').trim();
  ...
  if (automationId && user) {
    ...
    // Helper function to clean config values
    const cleanConfig = (config) => {
      const cleaned = {};
      for (const [key, value] of Object.entries(config)) {
        if (typeof value === 'string') {
          // Remove any [ACTIVE SETUP: ...] blocks and other system prompts
          cleaned[key] = value.replace(/\s*\[ACTIVE SETUP:[\s\S]*?\]\s*/g, '').trim();
        } else {
          cleaned[key] = value;
        }
      }
    ...
    const existingConfig = cleanConfig({
    ...
    const mergedConfig = {
      ...existingConfig,
      [args.field_name.toUpperCase()]: cleanValue
    };
