  ...
  try {
    ...
    if (args.automation_id && args.automation_id.length > 10) {
      const { data, error } = await supabase
        .from('automations')
        .select('id, is_active, required_inputs, required_connectors, name, description, workflow, activepieces_source_flow_id, activepieces_source_project_id, activepieces_trigger_type')
...
export async function handleAutoSetupWithExisting(args, existingConfig, user, controller, encoder) {
  ...
  try {
    ...
    const { data: automation, error } = await supabase
      .from('automations')
      .select('id, required_inputs, required_connectors, name, workflow, system_config, activepieces_source_flow_id, activepieces_source_project_id, activepieces_trigger_type')
      .eq('id', args.automation_id)
      .single();
    ...
    if (usesActivepieces && lifecycle.postSetupAction === 'configure_publish_wait') {
