...
import { listAutomationConnectionStatus, humanizePiece } from '@/lib/activepieces/connections';
...
// Not real apps — these never produce a user-visible connection.
const PSEUDO_CONNECTORS = new Set(['manual', 'webhook', 'schedule', 'trigger']);

// Turn required connector slugs into a readable list: "Gmail and Google Sheets".
function describeConnectorList(connectors) {
  const labels = [...new Set(
    (Array.isArray(connectors) ? connectors : [])
      .filter((connector) => !PSEUDO_CONNECTORS.has(
        String(connector || '').replace('@activepieces/piece-', '').toLowerCase().trim()
      ))
      .map((connector) => humanizePiece(connector))
      .filter((label) => label && label !== 'Connection')
  )];

  if (labels.length === 0) return null;
  if (labels.length === 1) return { label: labels[0], count: 1 };
  if (labels.length === 2) return { label: `${labels[0]} and ${labels[1]}`, count: 2 };
  return { label: `${labels.slice(0, -1).join(', ')}, and ${labels.at(-1)}`, count: labels.length };
}

function getSetupErrorMessage(error, fallback, connectors = []) {
  ...
  if (
    error?.status === 401 &&
    (error?.message === 'INVALID_CREDENTIALS' || error?.data?.code === 'INVALID_CREDENTIALS')
  ) {
    const described = describeConnectorList(connectors);
    const reassurance = described
      ? `Your ${described.label} connection${described.count === 1 ? ' was' : 's were'} not changed.`
      : 'None of your connected accounts were changed.';
    return `ModelGrow’s automation runtime needs its service connection repaired before I can finish setup. ${reassurance}`;
  }
  return fallback;
}
...
export async function handleStartSetup(args, user, controller, encoder) {
  ...
  // Hoisted so the catch below can name the exact apps this automation needs.
  let setupConnectors = [];
  ...
  try {
    ...
    setupConnectors = requiredConnectors;
    ...
    if (requiredInputs.length > 0) {
      ...
      // `every` on an empty list is already true, which is the right answer for
      // an automation that asks for nothing: there is no unfilled field. The
      // config-is-non-empty guard that used to sit in front of this overrode
      // that and sent zero-input automations down the collect-details path with
    ...
    sendSSE(controller, encoder, {
      ...
      automation_name: args.automation_name || null,
      reason: e?.code || e?.data?.code || e?.message || 'SETUP_FAILED',
    });
    sendSSE(controller, encoder, { content: getSetupErrorMessage(e, "Something went wrong. Please try again.", setupConnectors) });
    return false;
  }
}
...
export async function handleAutoSetupWithExisting(args, existingConfig, user, controller, encoder) {
  ...
  try {
    ...
    // The header is written after the list, not before it, because plenty of
    // automations declare no manual inputs at all — this one runs entirely off
    // its connected accounts. Announcing "here's what I have" and then
    // rendering an empty string reads as a broken response, so an automation
    // with nothing to confirm says so instead of promising a list it cannot
    // produce.
    if (summaryItems.length > 0) {
      ...
      const via = connectors.length > 0 ? ` It runs on your connected ${connectors.join(' and ')}.` : '';
      sendSSE(controller, encoder, { content: `\n✓ This one needs no setup details from you.${via}` });
    }
    ...
    sendSSE(controller, encoder, { content: getSetupErrorMessage(e, "Error during setup. Please try again.", setupConnectors) });
  }
}
