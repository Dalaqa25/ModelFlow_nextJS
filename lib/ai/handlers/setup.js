  ...
  try {
    ...
    if (args.automation_id && args.automation_id.length > 10) {
      const { data, error } = await supabase
        .from('automations')
        .select('id, is_active, required_inputs, required_connectors, name, description, activepieces_source_flow_id, activepieces_source_project_id, activepieces_trigger_type')
    ...
    if (usesActivepieces && requiredConnectors.length > 0) {
      ...
      const connectionStatus = await listRuntimeConnectionStatus({ supabase, user, automation });
      const missingConnections = connectionStatus.requirements.filter((requirement) => !requirement.connected);
      const targetConnections = missingConnections.length > 0 ? missingConnections : connectionStatus.requirements;
      const displayLabels = targetConnections.map((connection) => connection.displayName).filter(Boolean);
      const connectorLabel = displayLabels.length === 1
        ? displayLabels[0]
        : displayLabels.join(', ');
      const nextConnectionLabel = targetConnections[0]?.displayName || requiredConnectors[0] || 'the required app';

      sendSSE(controller, encoder, {
        content: `To continue, connect the required account${targetConnections.length === 1 ? '' : 's'}: ${connectorLabel || nextConnectionLabel}.\n\nI'll keep you inside ModelGrow and use Activepieces only in the background for the secure connection.`
      });
      ...
      sendSSE(controller, encoder, {
        ...
        automation_id: automationId,
        automation_name: automation.name,
        activepieces_project_id: runtimeFlow.activepieces_project_id,
        activepieces_flow_id: runtimeFlow.activepieces_flow_id,
