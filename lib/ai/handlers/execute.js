...
export async function handleExecuteAutomation(args, user, controller, encoder) {
  ...
  try {
    // VALIDATION STEP 1: If no config came through, try to recover what setup
    // saved. This is recovery only — whether anything is actually missing is
    // decided in step 3, which compares the config against the automation's
    // required_inputs and can name the specific fields.
    //
    // This used to reject an empty config outright, on the assumption that
    // empty meant unconfigured. Plenty of automations require no manual input
    // at all: they run entirely off connected accounts, so {} is their complete
    // and correct configuration. Those could be set up and then refused at the
    // last step, told to "set it up first" with nothing left to set up. An
    // empty config is not evidence of a problem — an unsatisfied required
    // input is, and only step 3 knows about those.
    if (!args.config || typeof args.config !== 'object') args.config = {};

    if (Object.keys(args.config).length === 0 && args.automation_id && user?.id) {
    ...
    if (usesActivepieces) {
      ...
      const spend = await recordSuccessfulTokenSpend({
        ...
        automation,
        tokenCost,
        executionId: activepiecesResult.activepieces.runId,
      });
    ...
    if (isActuallySuccessful) {
      ...
      const spend = await recordSuccessfulTokenSpend({
        ...
        tokenCost,
        engine: 'n8n-native',
        executionId: runnerResult.executionId || result.executionId,
      });
    ...
    if (
      e.code === 'ACTIVEPIECES_OAUTH_SCOPE_UNSUPPORTED' ||
      e.code === 'ACTIVEPIECES_SHARED_OAUTH_REQUIRED'
    ) {
      sendSSE(controller, encoder, {
        content: `⚠️ ${e.message}\n\nThis automation is outside the permissions supported by Activepieces shared OAuth. ModelGrow did not run it or charge tokens.`,
      });
      return;
    }
