...
export async function handleExecuteAutomation(args, user, controller, encoder) {
  ...
  try {
    ...
    const { data: automation, error } = await supabase
      .from('automations')
      .select('id, is_active, required_inputs, required_connectors, name, token_cost, author_email, requires_background, activepieces_source_flow_id, activepieces_source_project_id, activepieces_trigger_type')
    ...
    if (!usesActivepieces && requiredConnectors.length > 0) {
    ...
    if (usesActivepieces) {
      ...
      if (!activepiecesResult.success) {
        await supabase.from('automation_executions').insert({
          ...
          error_message: `Activepieces run status: ${activepiecesResult.activepieces.runStatus}`,
          metadata: { engine: 'activepieces', activepieces: activepiecesResult.activepieces },
        });

        sendSSE(controller, encoder, {
          content: `❌ Activepieces ran the automation but returned status: ${activepiecesResult.activepieces.runStatus}. Would you like to try again?`
        });
        return;
      }
    ...
    if (tokenCost > 0 && user?.id) {
      ...
      // Deduct tokens from runner (use public.users.id)
      ...
      if (deductError) {
        logError('[TOKEN] Failed to deduct tokens:', deductError);
        sendSSE(controller, encoder, { content: '⚠️ Failed to process token payment. Please try again.' });
        return;
      }

      log('[TOKEN] Deducted', tokenCost, 'tokens from runner. New balance:', runner.token_balance - tokenCost);
      ...
      await supabase.from('token_transactions').insert({
        ...
        metadata: {
          ...
          developer_email: fullAutomationData?.author_email
        }
      });
    } else {
    ...
    if (isActuallySuccessful) {
      ...
      // ============================================
      // TOKEN ECONOMY: Credit creator after successful run
      // Only credit if the runner is a DIFFERENT user than the creator
      // ============================================
      if (tokenCost > 0 && fullAutomationData?.author_email && user?.id && fullAutomationData.author_email !== user.email) {
        log('[TOKEN] Crediting creator:', fullAutomationData.author_email);
        const usdAmount = tokenCost * TOKEN_TO_USD;

        const { data: creator } = await supabase
          .from('users')
          .select('id, total_earnings_usd')
          .eq('email', fullAutomationData.author_email)
          .single();
        ...
        if (creator) {
          ...
          await supabase
            .from('users')
            .update({ 
              total_earnings_usd: (parseFloat(creator.total_earnings_usd) || 0) + usdAmount
            })
            .eq('id', creator.id);
          ...
          log('[TOKEN] Credited', tokenCost, 'tokens and $' + usdAmount.toFixed(2), 'to creator');
        }
      }
