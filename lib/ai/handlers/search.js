...
export async function handleSearchAutomations(args, controller, encoder) {
  ...
  try {
    ...
    const { data: allActive } = await supabase
      .from('automations')
      .select('id, name, description, required_inputs, required_connectors, token_cost')
      .eq('is_active', true);

    if (allActive && allActive.length > 0 && browseAll) {
      searchResults = allActive.slice(0, 6);
    ...
    if (!browseAll && searchResults.length === 0) {
      try {
