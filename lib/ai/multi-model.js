...
// Groq serves both roles now. GitHub Models answers 410
// github_models_retirement_brownout — a scheduled retirement rather than an
// outage — so classification would fail here exactly as tool argument
// generation did in the stream route, taking /api/ai/chat down with it.
const groqClient = new OpenAI({
  baseURL: "https://api.groq.com/openai/v1",
  apiKey: process.env.GROQ_API_KEY,
});

// Kept pointing at Groq rather than deleted: the two roles are separated on
// purpose, and splitting them again across providers should stay a one-line
// change.
const githubClient = groqClient;

// Both models must support JSON mode and tool calling, which llama-3.3 does.
