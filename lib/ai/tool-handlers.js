...
export async function handleCollectTextInput(args, user, controller, encoder, setupContext = null) {
  ...
  if (automationId && user) {
    ...
    // ALSO extract previously collected fields from setupContext (conversation history)
    // This prevents fields from being lost between tool calls
    if (setupContext && setupContext.conversationHistory) {
      const allMessages = setupContext.conversationHistory.join('\n');
      // Look for field_collected events and awaiting_input collected_config in conversation
