  ...
  try {
    ...
    if (executeResponse.ok) {
      ...
      if (automation.requires_background) {
        sendSSE(controller, encoder, {
          content: "\n\nThis automation requires to run on the background so it tracks what's going on as the automation is running. Would you like to make it run in the background?"
        });

        // Pass the config as context so the AI can use it when user says "Yes"
        // We use hidden_context so it's not shown to user but AI sees it
        sendSSE(controller, encoder, {
          type: 'hidden_context',
