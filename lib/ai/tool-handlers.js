...
export async function handleStartSetup(args, user, controller, encoder) {
  ...
  try {
    ...
    console.log('[handleStartSetup] Parsed inputs:', requiredInputs);
    console.log('[handleStartSetup] Parsed connectors:', requiredConnectors);
    ...
    // ======= STEP 2: ALL CONNECTORS DONE - Ask for inputs one at a time =======
    if (requiredInputs.length > 0) {
      // Find the first input that hasn't been collected yet
      const firstInput = requiredInputs[0];
      const fieldName = (firstInput.name || firstInput).toUpperCase();
      const friendlyName = fieldName.toLowerCase().replace(/_/g, ' ');
      const description = firstInput.description || friendlyName;

      // Choose the right intro based on whether we connected an account
      const introPhrase = requiredConnectors.length > 0
        ? "Your account is connected."
        : "";

      sendSSE(controller, encoder, {
        content: `${introPhrase} Now, I need to know ${description.toLowerCase().endsWith('?') ? description : description + '.'} Could you please provide ${friendlyName.includes('url') ? 'the URL' : 'this'}?`
      });

      // CRITICAL FIX: Emit awaiting_input so the frontend state knows what is missing!
      // Without this, the Safety Net on the frontend fails to catch AI omissions.
      sendSSE(controller, encoder, {
        type: 'awaiting_input',
        automation_id: automationId,
        automation_name: automation.name,
        missing_fields: requiredInputs.map(f => (f.name || f).toUpperCase()),
        collected_config: {}
      });
    } else {
      sendSSE(controller, encoder, {
...
function explainRequirements(inputs) {
  ...
  if (!inputs || !Array.isArray(inputs)) {
    console.log('[explainRequirements] inputs is not an array:', inputs);
    return [];
  }
...
export async function handleAutoSetup(args, user, controller, encoder, setupContext) {
  ...
  if (setupContext && setupContext.existingFiles) {
    console.log('[AUTO_SETUP] Found existing files in setupContext:', setupContext.existingFiles);
  ...
  if (setupContext && setupContext.conversationHistory) {
    ...
    if (foundCount === 0) {
      console.log('[AUTO_SETUP] Trying EXISTING_FILES pattern...');
      ...
      while ((match = existingFilesPattern.exec(allMessages)) !== null) {
        ...
        foundCount++;
        console.log('[AUTO_SETUP] Found existing file #' + foundCount + ':', fieldName, '=', filePath);
        existingConfig[fieldName] = filePath;
      }
      
      if (foundCount === 0) {
        console.log('[AUTO_SETUP] No EXISTING_FILES markers found');
        console.log('[AUTO_SETUP] Searching for pattern in:', allMessages.substring(allMessages.length - 500));
      }
    }

    // Pattern 3: Fallback - look for "Path: " after file upload markers
    if (foundCount === 0) {
      console.log('[AUTO_SETUP] Trying fallback pattern...');
      ...
      while ((match = fallbackPattern.exec(allMessages)) !== null) {
        ...
        const fieldName = 'VIDEO_FILES';
        foundCount++;
        console.log('[AUTO_SETUP] Found file upload (fallback) #' + foundCount + ':', fieldName, '=', filePath);
        existingConfig[fieldName] = filePath;
      }
    }
...
export async function handleCollectTextInput(args, user, controller, encoder, setupContext = null) {
  ...
  // Enforce LinkedIn safety logic on the frontend state
  if (fieldLower.includes('posting_interval')) {
    const valStr = String(args.value).toLowerCase();
    
    // Provide a pass-through for "once" since it's not a recurring rapid post
    // If the LLM passes strictly "1" or "0", assume it means a one-time execution and let it pass as "once"
    if (valStr.includes('once') || valStr.includes('one time') || valStr === '1 time' || valStr === '1' || valStr === '0') {
      args.value = 'once';
    } else {
      const numMatch = valStr.match(/\d+/);
      if (numMatch) {
        const hours = parseInt(numMatch[0], 10);
        if (hours < 12) {
          sendSSE(controller, encoder, {
            content: `\n\n⚠️ **LinkedIn Policy Guard:** I've adjusted your recurring interval to 12 hours to protect your account from LinkedIn's anti-spam restrictions.\n`
          });
          args.value = "12";
        } else {
          args.value = hours.toString();
        }
      }
      // If no digits and not 'once', we just keep the original args.value
    }
  }
  ...
  if (automationId && user) {
    ...
    if (setupContext && setupContext.conversationHistory) {
      ...
      while ((match = fieldCollectedPattern.exec(allMessages)) !== null) {
        ...
        if (!existingConfig[prevField]) {
          ...
          console.log(`[collect_text_input] Recovered field from history: ${prevField} = ${prevValue}`);
        }
      }
    }
    ...
    console.log('[collect_text_input] Merged config for auto-setup:', mergedConfig);
    console.log('[collect_text_input] args.existing_config was:', args.existing_config);
    console.log('[collect_text_input] setupContext.collectedConfig was:', setupContext?.collectedConfig);
...
export async function handleListAutomationFiles(args, user, controller, encoder, setupContext) {
  console.log('[list_automation_files] Called with args:', args);
  console.log('[list_automation_files] setupContext received:', setupContext ? 'YES' : 'NO');
  ...
  try {
    ...
    console.log('[list_automation_files] Listing files:', {
      user_id: user.id,
      automation_id,
      bucket: bucketName,
      folder: folderPath
    ...
    if (listError) {
      console.error('[list_automation_files] Error:', listError);
      sendSSE(controller, encoder, {
    ...
    if (setupContext) {
      ...
      console.log('[list_automation_files] Updated setupContext with existing files:', setupContext.existingFiles);
    }

  } catch (error) {
    console.error('[list_automation_files] Error:', error);
    sendSSE(controller, encoder, {
...
export async function handleDeleteAutomationFile(args, user, controller, encoder) {
  console.log('[delete_automation_file] Called with args:', args);
  ...
  try {
    ...
    console.error('[delete_automation_file] Error:', error);
    sendSSE(controller, encoder, {
...
export async function handlePreviewAutomationFile(args, user, controller, encoder) {
  console.log('[preview_automation_file] Called with args:', args);
  ...
  try {
    ...
    console.log('[preview_automation_file] Generating signed URL:', {
      user_id: user.id,
      automation_id,
      bucket: bucketName,
      file_path: filePath
    ...
    if (signError) {
      console.error('[preview_automation_file] Error:', signError);
      sendSSE(controller, encoder, {
    ...
    console.error('[preview_automation_file] Error:', error);
    sendSSE(controller, encoder, {
...
export async function handleExecuteAutomation(args, user, controller, encoder) {
  ...
  try {
    ...
    if (!args.config || Object.keys(args.config).length === 0) {
      console.log('[handleExecuteAutomation] Empty config from AI, attempting DB recovery...');
      ...
      if (args.automation_id && user?.id) {
        ...
        if (userAutomation?.parameters && Object.keys(userAutomation.parameters).length > 0) {
          console.log('[handleExecuteAutomation] Recovered config from DB:', Object.keys(userAutomation.parameters));
    ...
    console.log('[handleExecuteAutomation] Response status:', executeResponse.status);
    console.log('[handleExecuteAutomation] Response content-type:', contentType);
    
    if (!contentType || !contentType.includes('application/json')) {
      const text = await executeResponse.text();
      console.error('[handleExecuteAutomation] Non-JSON response:', text.substring(0, 500));
    ...
    if (isActuallySuccessful) {
      // Try to extract matched_jobs from the runner result
      const runnerResult = result.result || result;
      const outputs = runnerResult?.outputs || {};

      // Look for matched_jobs in the Parse AI JSON node output
      let matchedJobs = null;
      const parseAiNode = outputs['Parse AI JSON'] || outputs['parse-ai-json'];
      console.log('[handleExecuteAutomation] Parse AI JSON output:', JSON.stringify(parseAiNode));
      if (Array.isArray(parseAiNode) && parseAiNode[0]?.json?.matched_jobs) {
        matchedJobs = parseAiNode[0].json.matched_jobs;
      }

      if (matchedJobs && matchedJobs.length > 0) {
        let jobsMessage = `Found ${matchedJobs.length} best-matched jobs for you:\n\n`;
        matchedJobs.forEach((job, i) => {
          jobsMessage += `${i + 1}. ${job.title} at ${job.company}\n`;
          jobsMessage += `${job.location}`;
          if (job.salary && job.salary !== 'Not specified') jobsMessage += ` · ${job.salary}`;
          jobsMessage += `\n${job.ai_reasoning}\n`;
          jobsMessage += `[Apply here](${job.link})\n\n`;
        });
        sendSSE(controller, encoder, { content: jobsMessage });
      } else {
        sendSSE(controller, encoder, {
    ...
    console.error('[handleExecuteAutomation] Error:', e);
    console.error('[handleExecuteAutomation] Stack:', e.stack);
...
function parseRequiredInputs(inputs) {
  console.log('[parseRequiredInputs] Raw inputs:', inputs, 'Type:', typeof inputs);
  if (!inputs) return [];
  ...
  console.log('[parseRequiredInputs] Inputs is not an array, returning empty array');
  return [];
}
...
export async function handleScheduleAutomation(data, context, user, controller, encoder) {
  ...
  try {
    ...
    // Enforce LinkedIn frequency limits to prevent policy violations
    if (automation.name.toLowerCase().includes('linkedin') && schedule.type === 'recurring') {
      let isTooFrequent = false;
      const cronParts = schedule.cron.split(' ');
      
      // Check minutes field for */X or every minute
      if (cronParts[0].includes('*/') || cronParts[0] === '*' || (cronParts[0] !== '0' && cronParts[1] === '*')) {
        isTooFrequent = true;
      } 
      // Check hours field for */X
      else if (cronParts[1].includes('*/')) {
        const hours = parseInt(cronParts[1].replace('*/', ''), 10);
        if (hours < 12) isTooFrequent = true;
      }

      if (isTooFrequent) {
        console.log(`[handleScheduleAutomation] Overriding frequent schedule ${schedule.cron} to every 12 hours for LinkedIn policy.`);
        schedule.cron = '0 */12 * * *';
        schedule.humanReadable = 'Every 12 hours (Enforced by LinkedIn Anti-Spam Policy)';
        
        try {
          const cronParser = await import('cron-parser');
          const interval = (cronParser.default || cronParser).parseExpression(schedule.cron);
          schedule.nextRun = interval.next().toDate();
        } catch (e) {
          console.error("Failed to update nextRun:", e);
        }

        sendSSE(controller, encoder, {
          content: "To protect your account and comply with LinkedIn's anti-spam policies, I've adjusted the rapid posting schedule to run safely every 12 hours."
        });
        
        // Needs a slight delay before proceeding to give the user time to read the message, 
        // though sendSSE streams immediately so it's fine.
      }
    }
    ...
    if (process.env.SUPABASE_SERVICE_ROLE_KEY) {
      ...
      console.log('[handleScheduleAutomation] Added Supabase service key:', process.env.SUPABASE_SERVICE_ROLE_KEY ? 'EXISTS' : 'MISSING');
    } else {
      console.log('[handleScheduleAutomation] WARNING: SUPABASE_SERVICE_ROLE_KEY not found in environment');
    }
