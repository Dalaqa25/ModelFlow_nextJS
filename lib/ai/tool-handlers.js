...
export async function handleStartSetup(args, user, controller, encoder) {
  ...
  try {
    ...
    if (needsGoogle) {
      // First check if tokens exist for THIS specific automation
      ...
      console.log('[start_setup] Checking Google tokens for automation:', {
        user_id: user.id,
        automation_id: args.automation_id,
        hasTokensForThisAutomation: !!integration?.access_token
      });

      // If no tokens for this automation, check if user has tokens for ANY automation
      if (!integration || !integration.access_token) {
        ...
        console.log('[start_setup] Checking for ANY Google tokens:', {
          foundAnyTokens: !!anyIntegration?.access_token
        });

        if (anyIntegration && anyIntegration.access_token) {
          // User has Google connected for another automation - reuse those tokens
          console.log('[start_setup] Reusing existing Google tokens for new automation');
          await supabase
            .from('user_automations')
            .upsert({
              user_id: user.id,
          ...
          // Now we have tokens for this automation
          integration = anyIntegration;
        }
      }

      // If still no tokens, ask user to connect
      if (!integration || !integration.access_token) {
        console.log('[start_setup] No Google tokens found - requesting connection');
