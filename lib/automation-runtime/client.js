...
export async function callNativeAutomationRuntime(path, body, { timeoutMs = 60_000 } = {}) {
  ...
  if (!response.ok || data?.success === false) {
    const runtimeDetail = Array.isArray(data?.errors)
      ? data.errors.find((item) => typeof item === 'string' && item.trim())
      : null;
    const error = new Error(
      data?.error || data?.message || runtimeDetail || `Native n8n runtime failed (${response.status})`,
    );
...
export function certifyNativeAutomation({ automationId, userId = null }) {
  return callNativeAutomationRuntime('/api/automations/certify', {
    automation_id: automationId,
