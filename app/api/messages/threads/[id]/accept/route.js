...
import { notifyMessageAccepted } from '@/lib/messages/notify';
...
export async function POST(request, { params }) {
  try {
    ...
    if (thread.created_by_user_id === me.id) {
      return NextResponse.json(
        { error: 'You cannot accept your own message request' },
        { status: 403 }
      );
    }

    const updated = await messageDB.updateThreadStatus(threadId, 'active');

    await notifyMessageAccepted({
      recipientUserId: thread.created_by_user_id,
      accepterPublicUser: me,
      threadId,
    });

    return NextResponse.json({ threadId: updated.id, status: updated.status });
  } catch (error) {
    console.error('[messages accept]', error);
