        ...
        <div className="flex gap-6"> {/* This wrapper is required */}
            {plansData.map(plan => (
                <div key={plan.id} className='w-1/3 flex justify-center flex-col items-center border-gray-300 rounded-2xl border-1'>
                    ...
                    <ul className='text-gray-600 flex flex-col gap-1 mt-3 h-[90px] xl:text-2xl'>
                        {plan.features.map((feature, index) => (
                            <li key={index}>{feature}</li>
                        ))}
                    </ul>
                    <button className='text-lg btn-primary text-white w-[190px] py-3 rounded-xl xl:mt-6 mb-5'>
                        {plan.buttonText}
                    </button>
                </div>
            ))}
